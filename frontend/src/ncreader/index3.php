...
<head>
  ...
  <style>
    ...
    #parseBar {
      ...
      width: 0%;            
      background: #e74c3c;
      transition: width 0.2s linear; /* smooth animation */
    }
    ...
    <div id="main">
      ...
      <div id="map-container">
        ...
        <div id="mapLoading">
          ...
          <div style="margin-bottom:10px;">Parsing NetCDF...</div>
          
          <!-- Progress bar -->
          <div style="width:80%; height:20px; background:#444; border-radius:10px; overflow:hidden;">
            <div id="parseBar" style="height:100%; width:0%; background:#e74c3c;"></div>
          </div>
          
          <div id="parsePercent" style="margin-top:8px;">0%</div>
        </div>
  ...
  <script>
    ...
    document.getElementById('fileInput').addEventListener('change', async function (e) {
      ...
      try {
        ...
        const netcdfData = await parseNetCDFFile(file, pakistan_data);
        currentFileData = netcdfData;
        console.log("Clipped NetCDF:", netcdfData);
    ...
    //   async function parseNetCDFFile(file, boundaryGeoJSON) {
    ...
    //       try {
    //         if (!boundaryGeoJSON) {
    //           throw new Error("Pakistan boundary GeoJSON not loaded yet");
    //         }

    //         // Ensure we have a Polygon/MultiPolygon feature
    //         let boundaryFeature = boundaryGeoJSON;
    ...
    //         // Parse NetCDF
    //         const reader = new window.netcdfjs(arrayBuffer);
    //         // Extract variables
    //         const latVar = reader.variables.find(
    //           (v) => v.name === "lat" || v.name === "latitude" || v.name === "y"
    //         );
    //         const lonVar = reader.variables.find(
    //           (v) => v.name === "lon" || v.name === "longitude" || v.name === "x"
    //         );
    //         const timeVar = reader.variables.find((v) => v.name === "time");

    //         if (!latVar || !lonVar) {
    //           throw new Error("Lat/Lon variables not found in NetCDF file");
    //         }

    //         const lats = reader.getDataVariable(latVar);
    //         const lons = reader.getDataVariable(lonVar);
    //         const times = timeVar ? reader.getDataVariable(timeVar) : null;

    //         // Data variable (assuming SPI or first available data var)
    //         let spiVar = reader.variables.find((v) => v.name === "spi");
    //         if (!spiVar) {
    //           const dataVars = reader.variables.filter(
    //             (v) => !["lat", "latitude", "lon", "longitude", "time", "x", "y"].includes(v.name)
    //           );
    //           if (dataVars.length > 0) {
    //             spiVar = dataVars[0];
    //             showDebugInfo(`Using data variable: ${spiVar.name}`);
    //           } else {
    //             throw new Error("No data variable found in NetCDF file");
    //           }
    //         }
    //         const spi = reader.getDataVariable(spiVar);

    //         // Dimensions
    //         const timeDim = reader.dimensions.find((d) => d.name === "time");
    //         const latDim = reader.dimensions.find((d) => d.name === "lat" || d.name === "y");
    //         const lonDim = reader.dimensions.find((d) => d.name === "lon" || d.name === "x");
    ...
    //               let value = idx < spi.length ? spi[idx] : NaN;

    //               const point = turf.point([lons[j], lats[i]]);
    //               let isInside = false;
    ...
    //                 // Loop over multipolygon parts
    //                 for (const coords of boundaryFeature.geometry.coordinates) {
    ...
    //               // Keep only points inside Pakistan
    //               if (!isInside) value = NaN;
    //               row.push(value);
    //             }
    //             timeSlice.push(row);
    //             // 🔹 update progress
    //             processed++;
    //             if ((i % 10 === 0) && (t % 2 === 0)) {
    //             setProgress(Math.round(((t * latSize + i) / (timeSize * latSize)) * 100));
    ...
    async function parseNetCDFFile(file, boundaryGeoJSON) {
      return new Promise((resolve, reject) => {
        ...
        netcdfReader.onload = async function (e) {   // 🔹 make onload async
          try {
            if (!boundaryGeoJSON) {
              throw new Error("Pakistan boundary GeoJSON not loaded yet");
            }

            // Ensure Polygon/MultiPolygon feature
            let boundaryFeature = boundaryGeoJSON;
            if (boundaryGeoJSON.type === "FeatureCollection") {
              boundaryFeature = boundaryGeoJSON.features[0];
            }
            ...
            if (!latVar || !lonVar) throw new Error("Lat/Lon variables not found");

            const lats = reader.getDataVariable(latVar);
            const lons = reader.getDataVariable(lonVar);
            const times = timeVar ? reader.getDataVariable(timeVar) : null;

            // Data variable (SPI or fallback)
            let spiVar = reader.variables.find((v) => v.name === "spi");
            if (!spiVar) {
              const dataVars = reader.variables.filter(
                (v) => !["lat", "latitude", "lon", "longitude", "time", "x", "y"].includes(v.name)
            ...
            for (let t = 0; t < timeSize; t++) {
              ...
              for (let i = 0; i < latSize; i++) {
                ...
                processed++;
                if (i % 10 === 0) {
                  const percent = Math.round(((t * latSize + i) / (timeSize * latSize)) * 100);
                  setProgress(percent);
        ...
        netcdfReader.onerror = () => reject(new Error("Failed to read file"));
        netcdfReader.readAsArrayBuffer(file);
      });
    }
    ...
    function setProgress(percent) {
      ...
      const text = document.getElementById("parsePercent");
