...
function SentinelLULC(props) {
  ...
  useEffect(() => {
    async function fetchMyAPI() {
      try {
        const response = await Axios.post("../python/getlulc", {
          params: {
            unit: "National",
          },
        });

        console.log("LULC Response:", response.data);
        ...
        if (response.data.data && response.data.data.trim() !== "") {
          try {
            ...
            console.log("LULC parse error:", parseError);
            error("Error parsing LULC data from server");
          }
        } else {
          console.log("LULC no valid data, checking for errors");
          ...
          if (response.data.error && response.data.error.trim() !== "") {
            ...
            if (!containsSurvey) {
              console.log("LULC showing error:", response.data.error);
              error(response.data.error);
            } else {
              console.log("LULC filtered out survey error:", response.data.error);
            }
            // If it contains survey keywords, don't show any error
          } else {
            console.log("LULC no meaningful error to show");
          }
        ...
        if (err.response) {
          error("Failed to load LULC layer: Server error");
        } else if (err.request) {
          error("Failed to load LULC layer: Network error");
        } else {
          error("Failed to load LULC layer: " + err.message);
        }
      }
    }
  ...
  if (data) {
    return (
      <Pane name="Sentinel LULC" className="GridcellsPane">
        <TileLayer
          url={
